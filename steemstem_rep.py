       ...
       if os.path.isfile(os.path.join(os.getcwd(), comment_filename)):
           ...
           for line in comment_file:
               if 'null' in line:
                   ...
                   cd_time= (datetime.now() - datetime.strptime(line.split()[3]+'T'+line.split()[4], "%Y-%m-%dT%H:%M:%S")).total_seconds()
    ...
    for single_vote in all_votes:
        ...
        ## If not in the backup file, we read from the blockchain
            ...
            for single_comment in all_comments:
                ...
                if len(single_comment['body'])>comment_spam_limit and (delta_time-comment_delta_time)<comment_timelimit:
                    V_comment = math.sqrt(len(single_comment['body']))*math.exp(-math.log(2.)*comment_delta_time/half_life_comment)
                else:
                    V_comment = 0.
